...
```
GR-13 FLEET-IMPORT :
  Tout package .ivs importé DOIT être validé par ModelValidator
  avant activation sur l'unité cible.
  Si validation fail → rollback complet (produit supprimé).
  JAMAIS activer un modèle importé sans validation.

  Import réseau OU USB — jamais les deux simultanément.
  Si conflit → second bloqué + message "Import en cours".

  Master Unit : DEFERRED — ne pas implémenter en v7.0.
  Commentaire obligatoire : # RESERVED — Master Unit — IVS v8.0
```
...
```python
# ❌ INTERDIT — Activer sans validation (GR-13)
def import_package(self, path):
    shutil.copytree(tmp / "models", dest / "models" / "active")
    # ← INTERDIT : activation directe sans ModelValidator

# ✅ CORRECT — GR-13
def import_package(self, path):
    shutil.copytree(tmp / "models", dest / "models" / "active")
    validation = self._model_validator.validate(product_id)
    if not validation.passed:
        shutil.rmtree(dest)    # rollback
        raise FleetImportError(f"Validation failed: {validation.details}")

# ❌ INTERDIT — Import réseau + USB simultanés (GR-13)
# simultaneous_import = True

# ❌ INTERDIT — Implémenter Master Unit en v7.0
class MasterUnit:  # ← INTERDIT en v7.0
    ...

# ✅ CORRECT — Placeholder DEFERRED
...
## Structure Fleet v7.0

```
core/fleet_manager.py      ← export/import + validation GR-13
core/usb_monitor.py        ← USB auto-detect thread daemon
ui/screens/fleet_screen.py ← Fleet UI Tab

Package .ivs :
  package.json + config.json + calibration/ + models/ + logos/

Gates Fleet :
  G_FLEET-A : export + import réseau + GR-13
  G_FLEET-B : import USB + concurrent lock
  G_FLEET-C : DEFERRED (grep Master = vide)
```

---

## Vérification conformité Fleet

```bash
# Vérifier GR-13 respecté partout
grep -r "import_package\|import_via_usb" ts2i_ivs/ \
  --include="*.py" | grep -v "model_validator\|test_\|#"
# → Toute importation DOIT appeler model_validator

# Vérifier Master Unit absent
grep -r "MasterUnit\|master_unit\|fleet_master" ts2i_ivs/ \
  --include="*.py" | grep -v "RESERVED\|DEFERRED\|#"
# → Doit être vide
```
